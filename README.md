...
5. **ポートフォリオ2（NISA）**（🔒パスワード保護） … NISA口座で購入した別の保有銘柄の最新価格と評価コメント。パスワードは「shisan73」。あわせて、実家（岡山県備前市、ご両親2人・住民税非課税世帯）のキッチンリフォームに使えそうな補助金・助成金情報も掲載（動きの少ないテーマのため2週間に1回程度の更新）。
...
- タスクの内容: ①各カテゴリー（スピントロニクス・IOWN／AI認証・セキュリティ／日本経済ニュース／B型支援／ポートフォリオ／ポートフォリオ2）の情報をウェブ検索・株価取得 → ②`index.html` 内のJSONを更新 → ③GitHubへpush → ④チャットで内容を報告。
- ポートフォリオの保有銘柄・保有数は固定で、株価と評価コメントのみ毎回更新されます。チャットでの報告には、プライバシーの観点からポートフォリオの詳細（銘柄名・金額など）は書かれません。ポートフォリオ2内の実家リフォーム補助金情報も同様に詳細はチャットに書かれません。
